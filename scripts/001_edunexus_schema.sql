...
-- ============================================================================
-- PHASE 1: TABLE DEFINITIONS (Created first to avoid dependency errors)
-- ============================================================================
...
-- ============================================================================
-- PHASE 3: RLS POLICIES (Safe to reference tables now!)
-- ============================================================================
...
create policy "System can create profiles during signup" on public.profiles for insert to authenticated with check (true);
...
create policy "Documents are viewable by owner or evaluator" on public.research_documents for select to authenticated
  using (auth.uid() = owner_id or exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'faculty'));

drop policy if exists "Owners can insert documents" on public.research_documents;
create policy "Owners can insert documents" on public.research_documents for insert to authenticated with check (auth.uid() = owner_id);

drop policy if exists "Owners can update documents" on public.research_documents;
create policy "Owners can update documents" on public.research_documents for update to authenticated using (auth.uid() = owner_id) with check (auth.uid() = owner_id);
...
create policy "Sections are viewable by instructor or members" on public.class_sections for select to authenticated
  using (auth.uid() = instructor_id or exists (select 1 from public.section_enrollments enrollments where enrollments.section_id = class_sections.id and enrollments.student_id = auth.uid()));

drop policy if exists "Instructors can create sections" on public.class_sections;
create policy "Instructors can create sections" on public.class_sections for insert to authenticated with check (auth.uid() = instructor_id);

drop policy if exists "Instructors can update own sections" on public.class_sections;
create policy "Instructors can update own sections" on public.class_sections for update to authenticated using (auth.uid() = instructor_id) with check (auth.uid() = instructor_id);

drop policy if exists "Instructors can delete own sections" on public.class_sections;
create policy "Instructors can delete own sections" on public.class_sections for delete to authenticated using (auth.uid() = instructor_id);
...
create policy "Nodes are viewable by owner" on public.concept_nodes for select using (auth.uid() = owner_id);

drop policy if exists "Owners can insert nodes" on public.concept_nodes;
create policy "Owners can insert nodes" on public.concept_nodes for insert with check (auth.uid() = owner_id);

drop policy if exists "Owners can update nodes" on public.concept_nodes;
create policy "Owners can update nodes" on public.concept_nodes for update using (auth.uid() = owner_id) with check (auth.uid() = owner_id);
...
create trigger set_research_documents_updated_at before update on public.research_documents for each row execute function public.set_updated_at();

drop trigger if exists set_class_sections_updated_at on public.class_sections;
create trigger set_class_sections_updated_at before update on public.class_sections for each row execute function public.set_updated_at();

drop trigger if exists set_section_enrollments_updated_at on public.section_enrollments;
create trigger set_section_enrollments_updated_at before update on public.section_enrollments for each row execute function public.set_updated_at();

drop trigger if exists set_concept_nodes_updated_at on public.concept_nodes;
create trigger set_concept_nodes_updated_at before update on public.concept_nodes for each row execute function public.set_updated_at();
...
create policy "Allow QU users to upload and overwrite" on storage.objects for all to authenticated using (bucket_id = 'documents') with check (bucket_id = 'documents');

drop policy if exists "Allow authorized users to view files" on storage.objects;
create policy "Allow authorized users to view files" on storage.objects for select to authenticated
  using (bucket_id = 'documents' and (auth.uid() = owner or exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'faculty')));