...
export function Topbar({
  ...
  //const { profileName: sessionProfileName } = useUserSession()
  const { title, crumb } = TITLES[view]
  //const displayName = name ?? sessionProfileName
  return (
    <header className="sticky top-0 z-20 flex items-center justify-between gap-4 border-b border-border bg-card/80 px-5 py-3 backdrop-blur">
      <div className="min-w-0">
        ...
        {/* {displayName && <p className="truncate text-xs text-muted-foreground">Signed in as {displayName}</p>} */}
      </div>
